...
class ExperienceController extends Controller
{
    ...
    public function _getSearchResult()
    {
        ...
        if(!isset($post_data['number']) || $post_data['number'] == 10){
            $post_data['number'] = 12;
        }
        ...
        $html = '';
        if ($data['total'] > 0) {
            $column = (!isset($post_data['showmap']) || (isset($post_data['showmap']) && $post_data['showmap'] == 'no')) ? 'col-6 col-sm-6 col-md-2' : 'col-6 col-sm-6 col-md-4 col-lg-3 col-xl-3';
        ...
        $pag = view('frontend.experience.search.search_pag', [
            ...
            'page' => $post_data['page'],
            'number' => isset($post_data['number']) ? intval($post_data['number']) : 12
        ])->render();
                        ...
                        if($field['id'] == 'post_title'){
                            ...
                            if(!empty($langs)){
                                foreach ($langs as $kl => $vl) {
                                    $post_title_val .= '[:' . $vl . ']';
                                    $vvv = request()->get('post_title_' . $vl);
                                
                                    if(strpos($vvv, 'New Experience') !== false){
                                        $vvv = request()->get('post_title_' . $langs[0]);
                                    }
                                    $post_title_val .= $vvv;
                                }
                                $post_title_val .= '[:]';
                                $data[$field['id']] = $post_title_val;
                            }
                        }
